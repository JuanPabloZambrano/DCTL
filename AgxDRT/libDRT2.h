...
float Gangle = _atan2f(Gxy.y,Gxy.x)<0?_atan2f(Gxy.y,Gxy.x)+ 2*pi:_atan2f(Gxy.y,Gxy.x);
float Rangle = _atan2f(Rxy.y,Rxy.x)<0?_atan2f(Rxy.y,Rxy.x)+ 2*pi:_atan2f(Rxy.y,Rxy.x);
float Bangle = _atan2f(Bxy.y,Bxy.x)<0?_atan2f(Bxy.y,Bxy.x)+ 2*pi:_atan2f(Bxy.y,Bxy.x);
...
XYZ.x = (XYZin.x-whitepoint.x)/XYZin.y;
XYZ.y = (XYZin.y-whitepoint.y)/XYZin.y;
XYZ.z = XYZin.z;
float divxy = XYZ.y/XYZ.x;

float angle = _atan2f(XYZ.y,XYZ.x)<0.0f?_atan2f(XYZ.y,XYZ.x)+ 2*pi:_atan2f(XYZ.y,XYZ.x);
...
float3 Out = make_float3(angle,Cin,XYZin.z);
...
__DEVICE__ float tonescale(float in, float sp, float tp, float Pslope, float px, float py)
  {
...
__DEVICE__ float LintoLog(float in,float mn, float mx)
  {
    in = (_logf(in/0.18)/_logf(2)-mn)/(mx-mn);
    in = _fminf(1,in);
    in = _fmaxf(0.001,in);
  return in;
...
//High Rail
__DEVICE__ float3 Highrail(float3 in,float lumr,float lumg,float lumb)
{
  float g =_fmaxf(_fmaxf(in.x,in.y),in.z);
  float Y =  in.x*(lumr)+in.y*(lumg)+in.z*(lumb);
  float dech = (g-1)/(Y*0.5+(g-Y));
  float delum = 0.5*dech;

  in.x = g>1?(1-delum)*(Y)+(1-dech)*(in.x-Y):in.x;
  in.y = g>1?(1-delum)*(Y)+(1-dech)*(in.y-Y):in.y;
  in.z = g>1?(1-delum)*(Y)+(1-dech)*(in.z-Y):in.z;
...
__DEVICE__ float3 Lowrail(float3 in,Chromaticities N)
{
   ...
   float Y2 = (in.x-mn)*(lumc.x)+(in.y-mn)*(lumc.y)+(in.z-mn)*(lumc.z);

   in.x = mn<0? (in.x-mn)*Y/Y2:in.x;
   in.y = mn<0? (in.y-mn)*Y/Y2:in.y;
   in.z = mn<0? (in.z-mn)*Y/Y2:in.z;
   //in = maxf3(0,in);
