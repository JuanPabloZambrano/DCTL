...
__DEVICE__ float3 ClipGamut(float3 rgb,Chromaticities Ingamut){
...
__DEVICE__ float3 ComplimentLowGuard(float3 in,Chromaticities N)
{
  ...
  float md = (in.x+in.y+in.z)-mn-mx;
  float offset = _fmaxf(-mn,0.0);
  //float offset = -mn;

  float Y = (in.x*lumc.x)+(in.y*lumc.y)+(in.z*lumc.z);
  float3 compliment = make_float3(mx-in.x,mx-in.y,mx-in.z);
  float YCompliment = (compliment.x*lumc.x)+(compliment.y*lumc.y)+(compliment.z*lumc.z);
  float maxcomple = _fmaxf(compliment.x, _fmaxf(compliment.y, compliment.z));

  YCompliment= mn<0?maxcomple-YCompliment:Y;
  ...
  //Only offset negative values
  rgb = mn<0?rgb:in;
  float YOut = (rgb.x)*(lumc.x)+(rgb.y)*(lumc.y)+(rgb.z)*(lumc.z);

  //float luminanceratio =_fabs(1-YCompliment)/YOut;




  //float Yb = Y;
  //float absY = _fabs(in.x*lumc.x)+_fabs(in.y*lumc.y)+_fabs(in.z*lumc.z);

  //YCompliment = Y<=0 && mx>0 ?YCompliment:Y;
  //YCompliment = mn<=0 && mx>0 ?YCompliment:Y;

  //float Yold = Y;
  //Y = Y <0 ? YCompliment-mx:Y;
  //Y = mn <0 ? YCompliment:Y;
  //YCompliment = _fmaxf(mx-YCompliment,Y);
  //float Yout = mn<0?(YCompliment):Yb;
  ...
  //float luminanceratio = Y2>Y? Y/Y2:1.0;
  //float luminanceratio =Y/Y2;
  //Y2 = mn<0?Y2:Y;
  //Y2 = Y<=0 ?YCompliment:Y;
   //float3 out;
   //Y2 = mx-mn;

   //out.x = mn<0?luminanceratio*rgb.x:rgb.x;
   //out.y = mn<0?luminanceratio*rgb.y:rgb.y;
   //out.z = mn<0?luminanceratio*rgb.z:rgb.z;

   //out = mx<0?make_float3(0,0,0):out;
    //float Yout = mn<0?mx-YCompliment:Y;

    //float Ratio = mn<0?Yout/YOut:Y;
   //out = rgb;

   //float Brightness = Y<0? YCompliment :Y+YCompliment*Y;

   //out = maxf3(0.0,out);
   //out = make_float3(mx-YCompliment,YCompliment,Y);
   //out = compliment;
   float3 out = make_float3((YCompliment),_fabs(Y),(YOut));
   
   return out;

}

__DEVICE__ float3 CalculateBrightness(float3 in,Chromaticities N)
{
  ...
  float maxcomple = _fmaxf(compliment.x, _fmaxf(compliment.y, compliment.z));
   ...
   float3 out = make_float3(Y+Y*(YCompliment),Y+Y*(YCompliment),Y+Y*(YCompliment));
   
   return out;

}

__DEVICE__ float3 CalculateComplimentChroma(float3 in,Chromaticities N)
{
  ...
  float maxcomple = _fmaxf(compliment.x, _fmaxf(compliment.y, compliment.z));
   ...
   float3 out = make_float3((YCompliment),(YCompliment),(YCompliment));
